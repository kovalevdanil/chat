...
{
    char *name = (char *)get_auth_info(id_form, NAME);
    int *ID = (int *)get_auth_info(id_form, ID_AUTH);
    ...
    while (1)
    {
        ...
        for (auto iter = Sockets.begin(); iter != Sockets.end(); iter++)
        {
            ...
            else if (RecvSize != 0)
            {
                ...
                {
                    if (!authorize(current_user, buffer))
                        send(*iter, notifications[REAUTHOR],
                             strlen(notifications[REAUTHOR]), MSG_NOSIGNAL);
                }
                else
                {
